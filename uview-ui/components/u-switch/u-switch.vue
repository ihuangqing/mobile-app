	...
	<view class="u-switch"  :class="[value == true ? 'u-switch--on' : '', disabled ? 'u-switch--disabled' : '']" @tap="onClick"
	 :style="[switchStyle]">
		<view class="u-switch__node node-class" :style="{
			...
			<text v-show="showText && !loading" :class="value == true ? 'u-font-24 u-type-success' : 'u-font-24 u-type-error'">{{ value == true ? okText : ngText }}
			</text>
			<slot name="icon"></slot>
		</view>
	</view>
...
<script>
	 ...
	 * @property {Boolean} showText 是否加载文字（默认false 默认文字 ok ng）
