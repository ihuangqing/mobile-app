	...
	<view>
		<view class="u-index-bar">
			...
			<view v-if="showSidebar" class="u-index-bar__sidebar" @touchstart.stop.prevent="onTouchMove"
				@touchmove.stop.prevent="onTouchMove" @touchend.stop.prevent="onTouchStop"
				@touchcancel.stop.prevent="onTouchStop">
				<view v-for="(item, index) in indexList" :key="index" class="u-index-bar__index"
					:style="{zIndex: zIndex + 1, color: activeAnchorIndex === index ? activeColor : ''}" :data-index="index">
					{{ item }}
				</view>
			</view>
			<view class="u-indexed-list-alert" v-if="touchmove && indexList[touchmoveIndex]" :style="{
				...
				<text v-if="complexList.length">
					{{complexList.filter(item => item.code === indexList[touchmoveIndex]) [0].label}}
				</text>
				<text v-else>{{indexList[touchmoveIndex]}}</text>
			</view>
		</view>
	</view>
</template>
		...
		methods: {
			...
			getActiveAnchorIndex() {
				...
				for (let i = this.children.length - 1; i >= 0; i--) {
					...
					const reachTop = sticky ? preAnchorHeight : this.initTop;
			...
			scrollToAnchor(index) {
				...
				if (anchor) {
					...
					uni.pageScrollTo({
						duration: 0,
						scrollTop: anchor.top + this.scrollTop - this.initTop,
					});
				}
			}
		}
