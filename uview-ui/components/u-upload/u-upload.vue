<template>
	<view class="u-upload" v-if="!disabled">
		...
		<u-modal v-model="showModal" content="您确定要删除此项吗？" :show-cancel-button="true" @confirm="modalConfirm"></u-modal>
	</view>
</template>
...
export default {
	...
	data() {
		return {
			...
			isInCount: true,
			uploading: false,
			showModal: false //自定义模态框显示隐藏，用于修改删除弹出窗
		};
	},
	watch: {
		fileList: {
	...
	methods: {
		...
		async modalConfirm() { //自定义方法，使用uview模态框代替uni.showModal
			const index = this.curDelIndex
		...
		deleteItem(index) {
			...
			if (this.customDel) {
				this.$emit('on-customRemove', index)
