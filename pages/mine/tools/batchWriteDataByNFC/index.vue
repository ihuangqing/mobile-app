...
<template>
	<view class="page-wrap sws-wh100">
		<view class="material-search sws-w100 u-p-20 u-flex">
			...
			<u-search class="search-input" placeholder="请输入设备编码" shape="square" v-model="keyword" search-icon=""
			...
			<view class="u-p-20">
				<u-card margin="0 0 20rpx 0" :border="false"
					...
					<view slot="foot" class="u-text-right">
						<u-button v-show="device.tag" class="btn-style sws-line-h-repair" size="medium" type="warning"
							@click="toDelTag(device, index)">解绑标签
						</u-button>
						<u-button v-show="!device.tag" class="btn-style sws-line-h-repair u-m-l-30" size="medium" type="success"
	...
	export default {
		...
		methods: {
			...
			delTag(device) { //解绑设备和标签
				this.$u.api.postUnbindTagForNFC(device.id).then(res => {
				...
				uni.navigateTo({
					...
					events: {
						acceptDataFromOpenedPage: function(res) { // 为指定事件添加一个监听器，获取被打开页面传送到当前页面的数据
							console.log('res:', res)
							_self.list[_self.curIndex].tag = res.nfcCode
						}
					}
				})
			},
			getDeviceList(deviceCode, flag = false) { //flag true用于通过设备编号精确查询 默认分页查列表
				...
				this.$u.api.getDeviceListForNFC(params).then(res => {
					if (res.msgCode === '000000') {
